...
namespace Assets.IE.V2
{
    public class LayerIE
    {
        ...
        public List<string> MaterialFamilies
        {
            get
            {
                var list = new List<string>(m_MaterialFamilyNum);
                for(int i = 0; i < m_MaterialFamilyNum; ++i)
                {
                    char[] name = new char[m_MaterialFamilyLengths[i]];
                    for (int j = 0; j < name.Length; ++j)
                        name[j] = (char)m_MaterialFamilyNames[i][j];
                    list.Add(new string(name, 0, name.Length));
                }
                return list;
            }
            set
            {
                m_MaterialFamilyNum = (byte)value.Count;
                m_MaterialFamilyLengths = new byte[m_MaterialFamilyNum];
                m_MaterialFamilyNames = new byte[m_MaterialFamilyNum][];
                for(int i = 0; i < m_MaterialFamilyNum; ++i)
                {
                    m_MaterialFamilyLengths[i] = (byte)value[i].Length;
                    m_MaterialFamilyNames[i] = new byte[m_MaterialFamilyLengths[i]];
                    for (int j = 0; j < m_MaterialFamilyLengths[i]; ++j)
                        m_MaterialFamilyNames[i][j] = (byte)value[i][j];
                }
            }
        }
        public List<float> MaterialFamilyChances
        {
            get
            {
                return new List<float>(m_MaterialFamilyChances);
            }
            set
            {
                m_MaterialFamilyNum = (byte)value.Count;
                m_MaterialFamilyChances = value.ToArray();
            }
        }
        ...
        public byte m_MaterialFamilyNum;
        public ushort m_PropNum;
        public ushort m_MonsterNum;
        public ushort m_EffectNum;
        public byte m_LayerNum;

        private byte[] m_MaterialFamilyLengths;
        private byte[][] m_MaterialFamilyNames;
        private float[] m_MaterialFamilyChances;
        ...
        public LayerIE()
        {
            ...
            Layer = 0;
            m_MaterialFamilyNum = 0;
            m_MaterialFamilyLengths = null;
            m_MaterialFamilyNames = null;
            m_MaterialFamilyChances = null;
        ...
        //    {
        //        Layer = Layer,
        //        MaterialTypes = new List<int>(m_MaterialFamilyNum),
        //        MaterialTypeChances = new List<float>(m_MaterialFamilyNum),
        ...
        //    if (!IsLinkedLayer)
        //    {
        //        var names = MaterialFamilies;
        //        for (int i = 0; i < m_MaterialFamilyNum; ++i)
        //        {
        //            info.MaterialTypes.Add(BlockMaterial.FamilyDict[names[i]]);
        //            info.MaterialTypeChances.Add(m_MaterialFamilyChances[i]);
        //        }
        ...
        public V3.LayerIE Upgrade()
        {
            var monsterFamilies = new List<NameChance>(m_MonsterNum);
            var propFamilies = new List<NameChance>(m_PropNum);
            var linkedLayers = new List<IDChance>(m_LayerNum);
            var materialFamilies = new List<NameChance>(m_MaterialFamilyNum);
            var oMatFamilies = MaterialFamilies;

            for(int i = 0; i < m_MaterialFamilyNum; ++i)
            {
                materialFamilies.Add(new NameChance()
                {
                    ID = oMatFamilies[i],
                    Chance = (ushort)Mathf.FloorToInt(m_MaterialFamilyChances[i] * 10000f)
                });
            }
            GameUtils.UpdateChances2(ref materialFamilies);
            for(int i = 0; i < m_LayerNum; ++i)
            {
                linkedLayers.Add(new IDChance()
                {
                    ID = m_LinkedLayers[i],
                    Chance = (ushort)Mathf.FloorToInt(m_LinkedLayerChances[i] * 10000f)
                });
            }
            GameUtils.UpdateChances2(ref linkedLayers);
            for (int i = 0; i < m_PropNum; ++i)
            {
                propFamilies.Add(new NameChance()
                {
                    ID = Assets.Props.PropFamilies[m_Props[i]].FamilyName,
                    Chance = (ushort)Mathf.FloorToInt(m_PropChances[i] * 10000f)
                });
            }
            GameUtils.UpdateChances2(ref propFamilies);
            for (int i = 0; i < m_MonsterNum; ++i)
            {
                monsterFamilies.Add(new NameChance()
                {
                    ID = Assets.Monsters.MonsterFamilies[m_Monsters[i]].Name,
                    Chance = (ushort)Mathf.FloorToInt(m_MonsterChances[i] * 10000f)
                });
            }
            GameUtils.UpdateChances2(ref monsterFamilies);

            var ie = new V3.LayerIE()
            {
                Slot = Layer,
                LayerType = (Def.BiomeLayerType)Layer,
                LinkedLayers = linkedLayers,
                Rotation = EnableRandomRotation ? Def.RotationState.COUNT : (Def.RotationState)DefaultBlockRotation,
                MaterialFamilies = materialFamilies,
                BlockHeight = BlockHeight,
                //MicroHeightOffset = Mathf.Clamp(BlockMicroHeightMax - BlockMicroHeightMin, 0f, 0.25f),
                MicroHeightMin = BlockMicroHeightMin,
                MicroHeightMax = EnableRandomMicroHeight ? BlockMicroHeightMax : BlockMicroHeightMin,
                BlockLengthMin = BlockLenghtMin,
                BlockLengthMax = EnableRandomBlockLenght ? BlockLenghtMax : BlockLenghtMin,
                PropChance = (ushort)(EnableRandomProps ? Mathf.FloorToInt(PropChance * 10000f) : 0),
                PropFamilies = propFamilies,
                PropSafetyDistance = PropNoSpawnRadius,
                MonsterChance = (ushort)(SpawnMonsters ? Mathf.FloorToInt(MonsterChance * 10000f) : 0),
                ...
                //BlockFloatSpeed = 0f,
                SemiVoidChance = 0,
                WideBlockChance = (ushort)Mathf.FloorToInt(RandomWideBlockChance * 10000f),
                StairBlockChance = (ushort)Mathf.FloorToInt(RandomStairBlockChance * 10000f)
            };
            return ie;
        }
        ...
        public static bool operator ==(LayerIE left, LayerIE right)
        {
            ...
            if (left.m_MaterialFamilyNum != right.m_MaterialFamilyNum)
                return false;

            for (int i = 0; i < left.m_MaterialFamilyNum; ++i)
            {
                if (left.m_MaterialFamilyLengths[i] != right.m_MaterialFamilyLengths[i])
                    return false;

                for (int j = 0; j < left.m_MaterialFamilyLengths[i]; ++j)
                {
                    if (left.m_MaterialFamilyNames[i][j] != right.m_MaterialFamilyNames[i][j])
                        return false;
                }

                if (left.m_MaterialFamilyChances[i] != right.m_MaterialFamilyChances[i])
                    return false;
            }
        ...
        public bool ReadLayer(ref FileStream fr)
        {
            ...
            m_MaterialFamilyNum = tempArray[lastIdx++];
            m_MaterialFamilyLengths = new byte[m_MaterialFamilyNum];
            m_MaterialFamilyNames = new byte[m_MaterialFamilyNum][];
            m_MaterialFamilyChances = new float[m_MaterialFamilyNum];
            ...
            {
                // ReadOnly the familyNamesLength
                tempArray = new byte[m_MaterialFamilyNum * 1];
                fr.Read(tempArray, 0, tempArray.Length);

                lastIdx = 0;
                int namesLength = 0;
                // MaterialFamilyLength
                for (int i = 0; i < m_MaterialFamilyNum; ++i)
                {
                    m_MaterialFamilyLengths[i] = tempArray[lastIdx++];
                    m_MaterialFamilyNames[i] = new byte[m_MaterialFamilyLengths[i]];
                    namesLength += m_MaterialFamilyLengths[i];
                }
                tempArray = new byte[m_MaterialFamilyNum * 4 + namesLength
                ...
                for(int i = 0; i < m_MaterialFamilyNum; ++i)
                {
                    for(int j = 0; j < m_MaterialFamilyLengths[i]; ++j)
                    {
                        m_MaterialFamilyNames[i][j] = tempArray[lastIdx++];
                    }
                }
                for (int i = 0; i < m_MaterialFamilyNum; ++i)
                {
                    m_MaterialFamilyChances[i] = BitConverter.ToSingle(tempArray, lastIdx);
                    lastIdx += 4;
                }
                // Props
                for (int i = 0; i < m_PropNum; ++i)
        ...
        //    tempArray[lastIdx++] = m_MaterialFamilyNum;
        ...
        //            namesLength += m_MaterialFamilyLengths[i];
        //        }
        //        tempArray = new byte[(1 + 4) * m_MaterialFamilyNum + namesLength
        //            + (2 + 4) * m_PropNum + (2 + 4) * m_MonsterNum
        //            + (2) * m_EffectNum];

        //        lastIdx = 0;
        //        // MaterialFamilyLengths
        //        for (int i = 0; i < m_MaterialFamilyNum; ++i)
        //        {
        //            tempArray[lastIdx++] = m_MaterialFamilyLengths[i];
        //        }
        //        // MaterialFamilyNames
        //        for(int i = 0; i < m_MaterialFamilyNum; ++i)
        //        {
        //            for(int j = 0; j < m_MaterialFamilyLengths[i]; ++j)
        //            {
        //                tempArray[lastIdx++] = m_MaterialFamilyNames[i][j];
        //            }
        //        }
        //        for (int i = 0; i < m_MaterialFamilyNum; ++i)
        //        {
        //            tempBytes = BitConverter.GetBytes(m_MaterialFamilyChances[i]);
        //            for (int j = 0; j < tempBytes.Length; ++j)
        //            {
        //                tempArray[lastIdx++] = tempBytes[j];
        //            }
        ...
        public override int GetHashCode()
        {
            ...
            hashCode = hashCode * -1521134295 + m_MaterialFamilyNum.GetHashCode();
            hashCode = hashCode * -1521134295 + EqualityComparer<byte[]>.Default.GetHashCode(m_MaterialFamilyLengths);
            hashCode = hashCode * -1521134295 + EqualityComparer<byte[][]>.Default.GetHashCode(m_MaterialFamilyNames);
            hashCode = hashCode * -1521134295 + EqualityComparer<float[]>.Default.GetHashCode(m_MaterialFamilyChances);
