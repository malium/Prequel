...
namespace Assets.IE.V1
{
    public class StructureIE
    {
        ...
        //public void ToStructure(ref CStrucEdit struc, bool copyID = true)
        //{
        //    if (copyID)
        ...
        //            var pilar = pilarGO.AddComponent<CPilar>();
        ...
        public static StructureIE FromFile(string path)
        {
            ...
            if (layerNum < 0 || layerNum > Def.MaxLayerSlots)
            {
                fr.Close();
            ...
            lastIdx += 2;
            if (blockNum < 0 || blockNum == ushort.MaxValue /*blockNum >= (StructureComponent.Width * StructureComponent.Height * StructureComponent.LayerAmount)*/)
            {
                fr.Close();
        ...
        public static V2.StructureIE LoadAndUpgrade(string path)
        {
            var sie = FromFile(path);
            return sie.Upgrade();
        }

        public V2.StructureIE Upgrade()
        {
            var olayers = Layers;
            var layers = new List<V2.LayerIE>(olayers.Count);
            for (int i = 0; i < olayers.Count; ++i)
                layers.Add(olayers[i].Upgrade());

            var oblocks = Blocks;
            var blocks = new List<V2.BlockIE>(oblocks.Count);
            for (int i = 0; i < oblocks.Count; ++i)
                blocks.Add(oblocks[i].Upgrade());


            var ie = new V2.StructureIE()
            {
                _FromFile = m_IsFromFile,
