...
{
	public class RunningState : IOddState
	{
		static readonly int AnimationHash = Animator.StringToHash("BaseLayer.Running");
		public override string StateName => "Running";
		public override OddState State => OddState.Running;
		const float AnimationLength = 2f;
		const float AnimationSpeed = 1f;
		const float AnimationTime = AnimationLength / AnimationSpeed;
		public const float OddMaxSpeed = 4f;
		...
		public override void OnUpdate()
		{
			...
			{
				if(Controller.ME.GetCurrentMoment() < WalkingState.OddMaxSpeed * 0.9f)
				{
