...
namespace Assets.UI
{
	public class StructureEditMenuUI : MonoBehaviour
	{
		public StructureEditUI EditUI;
		...
		public TMPro.TMP_Text SaveInvalidStrucNameText;
		public TMPro.TMP_InputField SaveStrucIF;
		...
		void AfterSelect()
		{
			...
			SaveButton.interactable = CanStrucBeSaved();
		...
		void ShowSaveDialog()
		{
			...
			var cleanName = GameUtils.CleanUniqueName(strucName);
		...
		void OnCrossButton()
		{
			...
			EditUI.enabled = true;
			if (!EditUI.CameraFreeButton.interactable)
		...
		{
			//value = GameUtils.CleanUniqueName(value);
			if (value.Length == 0)
			{
				SaveApplyButton.interactable = false;
				SaveInvalidStrucNameText.enabled = true;
			...
			string testName = GameUtils.GenerateUniqueName(value, 3, (string toCheck) =>
			{
				if (Structures.StrucDict.TryGetValue(toCheck, out int idxie))
					return idxie == EditUI.GetStruc().IDXIE;
				return true;
			});
			//int id = 0;
		...
		void OnSaveDont()
		{
			...
			if (!ie.IsFromFile())
			{
				...
				Structures.Strucs[idxie] = null;
				return;
			}
			ie = IE.V4.StructureIE.FromFile(ie.GetFilePath());
		...
		void ScreenshootPrepareTask()
		{
			...
			EditUI.OnCameraGameButton();
			EditUI.OnCameraEditButton();
		...
		private void OnEnable()
		{
			...
			SaveStrucIF.text = GameUtils.CleanUniqueName(strucName);
		}
		private void Awake()
		{
